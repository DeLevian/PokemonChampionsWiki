...
export class PokedexView {
    ...
    calculateEffectiveness(types) {
        ...
        const attackTypes = Object.keys(typeChart).filter(type => type !== 'stellar');
        const effectiveness = Object.fromEntries(attackTypes.map(type => [type, 1]));

        types.map(type => type.toLowerCase()).forEach(defenseType => {
            attackTypes.forEach(attackType => {
                effectiveness[attackType] *= typeChart[attackType]?.[defenseType] ?? 1;
            });
        });
